...
def main(argv):
    # param setting
    history_file = argv[1]
    future_file = argv[2] #why not use label?
    keyset_file = argv[3]
    ...
    for size in [10, 20]:
        for user in keyset['val']:
            pred = predicted_val[user]
        ...
        print(np.mean(hit))
        utility.append([size,np.mean(recall),np.mean(ndcg),np.mean(hit)])

    with open(f'/ivi/ilps/personal/yliu10/NBR-fairness/methods/tifuknn/tune/{num_nearest_neighbors}_{within_decay_rate}_{group_decay_rate}_{alpha}_{group_size}.txt', 'w') as f:
        for line in utility:
            f.write(str(line))
            f.write('\n')  
    ...
    pred_path = f'/ivi/ilps/personal/yliu10/NBR-fairness/methods/tifuknn/results/'+dataset_name + '_pred'+keyset_ind+'.json'
    pred_rel_path = f'/ivi/ilps/personal/yliu10/NBR-fairness/methods/tifuknn/results/'+dataset_name + '_rel'+keyset_ind+'.json'
