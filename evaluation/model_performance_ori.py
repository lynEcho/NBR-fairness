...
def get_repeat_eval(pred_folder, dataset, size, fold_list, file):
    ...
    for ind in fold_list:
        keyset_file = f'/ivi/ilps/personal/yliu10/NBR-fairness/keyset/{dataset}_keyset_{ind}.json'
...
def get_eval_input(pred_folder, dataset, fold_list, size, file, pweight):
    
    group_file = f'/ivi/ilps/personal/yliu10/NBR-fairness/methods/g-p-gp-topfreq/group_results/{dataset}_group_user.json'
    ...
    for ind in fold_list:
        ...
        default_results = MA.run_default_setting(listsize=size, pweight='default')
...
if __name__ == '__main__':
    ...
    eval_file = f'eval_{method_name}_default.txt'
    f = open(eval_file, 'w')
    for dataset in ['instacart', 'dunnhumby', 'tafeng']:
        ...
        get_eval_input(pred_folder, dataset, fold_list, 10, f, pweight='default')
        get_eval_input(pred_folder, dataset, fold_list, 20, f, pweight='default')
