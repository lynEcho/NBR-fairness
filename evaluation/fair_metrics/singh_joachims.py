...
def weighted_exposure(rec_df, group, weight_vector):
    ...
    #exp = group_df.T @ np.reciprocal(weight_vector) wrong!
   
    exp = group_df.T @ weight_vector
    ...
    # get the average over users, why? not /num_group
    try:
...
def discounted_gain(rec_df, group, weight_vector):
    ...
    #rec_df['group_discount'] = rec_df['rating'] / weight_vector wrong
    rec_df['group_discount'] = rec_df['rating'] * weight_vector 
    try:
        nusers = rec_df['user'].nunique()
        

        ctr = rec_df.groupby(group.category)['group_discount'].sum().reindex([group.major, group.minor], fill_value=0)
